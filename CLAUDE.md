# peaceful-lamport Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-03-19

## Active Technologies
- TypeScript 5.4 + React 18 (unchanged from 001) + React 18, Tailwind CSS 3, Vite 5 (unchanged from 001) (002-plot-exhaustion-maintenance)
- localStorage (browser — session persistence; schema version bumped to 2) (002-plot-exhaustion-maintenance)
- TypeScript 5.4 + React 18 + Vite 5, Tailwind CSS 3 (003-crop-disasters)
- localStorage (key: `pixel-parsnips-state`; schema version bumps 2 → 3) (003-crop-disasters)
- TypeScript ~5.6 + React 18.3 + React 18, Tailwind CSS 3.4, Vite 5.4 (no new deps added) (004-game-ui-revamp)
- localStorage (unchanged, schema version 3 — no changes) (004-game-ui-revamp)

- TypeScript 5.4 + React 18 + React 18, Tailwind CSS 3, Vite 5 (001-farming-tycoon-game)

## Project Structure

```text
src/
tests/
```

## Commands

npm test && npm run lint

## Code Style

TypeScript 5.4 + React 18: Follow standard conventions

## Recent Changes
- 004-game-ui-revamp: Added TypeScript ~5.6 + React 18.3 + React 18, Tailwind CSS 3.4, Vite 5.4 (no new deps added)
- 003-crop-disasters: Added TypeScript 5.4 + React 18 + Vite 5, Tailwind CSS 3
- 002-plot-exhaustion-maintenance: Added TypeScript 5.4 + React 18 (unchanged from 001) + React 18, Tailwind CSS 3, Vite 5 (unchanged from 001)


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
